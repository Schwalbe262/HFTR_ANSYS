...
REFERENCE_SCRIPT_FILE_NAME = f'D:\\script\\run_ansys_comp_ref_v2.py'
#REFERENCE_SCRIPT_FILE_NAME = "run_ansys_ref.py"
    

def run_simul(version_idx_str,N1,N2,space1,space2,l1,l2,h1,w1):
    ...
    os.mkdir(f'D:\script\ML_v2\SIMUL_comp_{version_idx_str}')
    ...
    with open(f'D:\script\ML_v2\SIMUL_comp_{version_idx_str}\info.yaml', "w") as info_file:
    ...
    with open(f'D:\\script\\ML_v2\\SIMUL_comp_{version_idx_str}\\run_ansys_{version_idx_str}.py',"w") as f :
    ...
    with open(f'D:\\script\\ML_v2\\SIMUL_comp_{version_idx_str}\\run_bat_{version_idx_str}.bat',"w") as f :
        ...
        f.write(f'"C:\\Program Files\\AnsysEM\\AnsysEM20.1\\Win64\\ansysedt.exe" -runscript "D:\\script\\ML_v2\\SIMUL_comp_{version_idx_str}\\run_ansys_{version_idx_str}.py"')

    workingDir = f'D:\script\ML_v2\SIMUL_comp_{version_idx_str}'
    executeFile = f'D:\script\ML_v2\SIMUL_comp_{version_idx_str}\\run_bat_{version_idx_str}.bat'
    os.chdir(workingDir)
    os.system(executeFile)
    
    with open(f'D:\script\ML_v2_data\Data_comp {version_idx_str}.csv',"r") as f :
        rdr = csv.reader(f)

        for line in rdr:
            a = 1
            
    with open(f'D:\script\ML_v2_data\Data_comp.csv',"a", encoding='utf-8', newline='') as f :
...
for i in range(1, 2): 
    ...
    try :
        run_simul(i,N1[i],N2[i],space1[i],space2[i],l1[i],l2[i],h1[i],w1[i])
        os.remove(f'D:\script\ML_aedt\ML_comp_v2.aedt.lock')
    except :
        time.sleep(1)	
